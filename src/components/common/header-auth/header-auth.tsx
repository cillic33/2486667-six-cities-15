...
import {useUpdateAllFavoriteStatuses} from '@/hooks/use-update-all-favorite-statuses/use-update-all-favorite-statuses';

function HeaderAuth() {
  const user = useAppSelector(usersSelectors.user);
  const favoriteCount = useFavoriteCount();
  const { logoutUser } = useActionCreators(usersActions);
  useUpdateAllFavoriteStatuses();
  ...
  return (
    <>
      <li className="header__nav-item user" data-testid="header-auth-li">
